...
void AdminConnector::onRequestReply(QtHttpRequest *request, QtHttpReply *reply){
    ...
    for(int i=0; i<waiting_array.size(); i++){
        ...
        tempLeft.transaction_number = temp["PIN"].toString();
        tempLeft.menu = temp["MENU_NAME"].toString();
...
//    }
    ...
    for(int i=0; i<complete_array.size(); i++){
        ...
        tempRight.transaction_number = temp["PIN"].toString();
        tempRight.menu = temp["MENU_NAME"].toString();
